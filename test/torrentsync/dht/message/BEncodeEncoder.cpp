...
TEST(BEncodeEncoder, constructor_destructor) { BEncodeEncoder enc; }

TEST(BEncodeEncoder, check_lexicographic_order) {
  BEncodeEncoder c;

  c.startDictionary();
  c.addDictionaryElement("a", "a");
  c.addDictionaryElement("c", "c");
  EXPECT_THROW(c.addDictionaryElement("b", "b"), std::logic_error);
}

TEST(BEncodeEncoder, add_integer) {
  BEncodeEncoder c;
  ...
  ASSERT_TRUE(c.value() == utils::makeBuffer("li12ee"));
}

TEST(BEncodeEncoder, bep005_ping_example_q) {
  BEncodeEncoder c;
...
TEST(BEncodeEncoder, bep005_ping_example_r) {
  BEncodeEncoder c;
...
TEST(BEncodeEncoder, bep005_find_node_example_q) {
  BEncodeEncoder c;
...
TEST(BEncodeEncoder, bep005_find_node_example_r) {
  BEncodeEncoder c;
