      ...
      if (structureStack.size() > 0 && structureStack.back().second == LIST) {
        const bool allDigits =
            std::all_of(structureStack.back().first.begin(),
                        structureStack.back().first.end(), isdigit);
        if (allDigits) {
          try {
...
utils::Buffer BEncodeDecoder::appendPath() {
  utils::Buffer buff;
  buff.reserve(16);
  std::for_each(structureStack.begin(), structureStack.end(),
                [&](const structureStackE &path) {
...
boost::optional<utils::Buffer>
BEncodeDecoder::find(const utils::Buffer &key) const {
