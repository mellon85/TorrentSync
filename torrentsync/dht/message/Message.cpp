...
const utils::Buffer Message::getTransactionID() const {
  ...
  if (!token)
    throw MessageException("Couldn't find token", ErrorType::protocolError);
  return *token;
}
...
const utils::Buffer Message::getID() const {
  ...
  auto id = find(path);
  if (!id)
    throw MessageException("Couldn't find peer id", ErrorType::protocolError);
  return *id;
}

const boost::optional<utils::Buffer>
Message::find(const utils::Buffer &key) const {
...
const std::string Message::string() const {
  ...
  std::for_each(_data.begin(), _data.end(), [&](const DataMap::value_type &_i) {
    message << pretty_print(_i.first) << ":" << pretty_print(_i.second)
            << std::endl;
  });
  return message.str();
}
