 ...
 */
template <size_t MaxSizeT> class NodeBucket : public boost::noncopyable {
public:
  //! constructor
  //! @param low the lower bound
  //! @param high the higher bound
  NodeBucket(const NodeData &low, const NodeData &high);
   ...
   */
  bool add(const std::shared_ptr<Node> addr);
   ...
   */
  bool remove(const Node &addr);

  /** Looks for a node in the bucket
   * @return a valid optional value if found or nothing
   * @param addr the address
   */
  const boost::optional<NodeSPtr> find(const NodeData &addr) const;
  ...
  bool inBounds(const std::shared_ptr<Node> &addr) const;
  bool inBounds(const NodeData &addr) const;
  ...
  inline const_iterator cend() const {
    return _elements.begin() + addressCount;
  }
...
NodeBucket<MaxSizeT>::NodeBucket(const NodeData &low, const NodeData &high)
    : low(low), high(high), addressCount(0) {}

template <size_t MaxSizeT> NodeBucket<MaxSizeT>::~NodeBucket() { clear(); }

template <size_t MaxSizeT> size_t NodeBucket<MaxSizeT>::size() const {
  return addressCount;
}

template <size_t MaxSizeT> size_t NodeBucket<MaxSizeT>::maxSize() const {
  return MaxSizeT;
}

template <size_t MaxSizeT> void NodeBucket<MaxSizeT>::clear() {
  std::for_each(begin(), end(), [](std::shared_ptr<Node> &t) {
    assert(t.get());
    t.reset();
  });
  addressCount = 0;
...
bool NodeBucket<MaxSizeT>::add(const std::shared_ptr<Node> addr) {
  ...
  if (size() < maxSize()) {
    ...
    iterator it = std::find_if(
        begin(), end(), bind1st(std::less<std::shared_ptr<Node>>(), addr));
...
template <size_t MaxSizeT> void NodeBucket<MaxSizeT>::removeBad() {
  const iterator it = std::remove_if(
      begin(), end(), [](const std::shared_ptr<Node> &addr) -> bool {
        assert(addr.get());
        return addr->isBad();
      });
...
template <size_t MaxSizeT>
bool NodeBucket<MaxSizeT>::inBounds(const std::shared_ptr<Node> &addr) const {
  assert(addr.get());
  return inBounds(*addr);
}

template <size_t MaxSizeT>
bool NodeBucket<MaxSizeT>::inBounds(const NodeData &addr) const {
  return low <= addr && addr <= high;
}

template <size_t MaxSizeT> bool NodeBucket<MaxSizeT>::remove(const Node &addr) {
...
std::ostream &operator<<(std::ostream &out,
                         const NodeBucket<MaxSizeT> &element) {
...
template <size_t MaxSizeT>
const boost::optional<NodeSPtr>
NodeBucket<MaxSizeT>::find(const NodeData &addr) const {
  const_iterator it = std::find_if(
      begin(), end(), [addr](const std::shared_ptr<Node> &bucket_addr) -> bool {
