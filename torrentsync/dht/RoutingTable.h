...
//! Number of addresses per batch while initialing the DHT, should be
//! configurable.
...
public:
  ...
  RoutingTable(boost::asio::io_service &io_service);
  ...
  void initializeNetwork(const udp::endpoint &endpoint);
...
protected:
  ...
  void sendMessage(const utils::Buffer &, const udp::endpoint &addr);

  //! @TODO
  void recvMessage(const boost::system::error_code &error, utils::Buffer buffer,
...
private:
  void processIncomingMessage(torrentsync::dht::message::AnyMessage &message,
  ...
  //! callbacks.
  boost::optional<Callback> getCallback(const message::AnyMessage &message);

  //! Registers a callback to be called when we receive a message.
  //! It will be executed only once and before any other processing.
  //! The callback will be removed in any case if it is not used in less than 60
  //! seconds.
  //! @param func is the function to call
  //! @param type mandatory parameter specifying the type of the message to
  //! signal.
  //!             must be from dht::messages::Messages.
  //! @param source optional parameter specifing if the message is awaited from
  //! a specific Peer
  //! @param transactionID optional parameter specifing if the message is
  //! awaited from a specific Peer
  void registerCallback(const Callback::callback_t &func,
                        const utils::Buffer &transactionID,
                        const boost::optional<dht::NodeData> &source =
                            boost::optional<dht::NodeData>());
  ...
  std::multimap<utils::Buffer, dht::Callback> _callbacks;
   ...
   */
  void sendError(const udp::endpoint &ip, const utils::Buffer &transaction,
                 const dht::message::ErrorType::error_type error);

  //! ************** Message handlers *****************

  //! Handle ping queries.
  void handleQuery(const dht::message::query::Ping &, const dht::Node &);
   ...
   */
  void handleReply(const dht::message::reply::Ping &, const dht::Node &);

  //! Handle find_node queries.
  void handleQuery(const dht::message::query::FindNode &, const dht::Node &);
   ...
   */
  void handleReply(const dht::message::reply::FindNode &, const dht::Node &);

  //! sends a ping message to the destination node (and setup a callback to
  //! receive).
...
void RoutingTable::load(Archive &ar, const unsigned int version) {
  ...
  // 1. ping all known and insert them in the routing table with the normal
  // procedure
  // 2. perform normal startup operation and let the bucket refreshing do it's
  // job
}
