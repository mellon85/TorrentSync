...
seeds_t getAndUpdateSeeds() {
  ...
  const size_t number_of_intervals =
      std::floor((now - last_time) / token_interval);
  if (number_of_intervals >= 1) {
    // shift by one to create space for a new one
    for (auto _i :
         boost::irange(static_cast<int>(_random_seeds.max_size()) - 1, 1, -1)) {
    ...
    std::fill(_random_seeds.end() - index_to_remove_from, _random_seeds.end(),
              _random_seeds[std::max(0ul, _random_seeds.size() -
...
utils::Buffer calculateToken(const udp::endpoint &endpoint,
...
utils::Buffer getToken(const udp::endpoint &endpoint) {
  return calculateToken(endpoint, getAndUpdateSeeds()[0]);
}

bool isTokenValid(const utils::Buffer &token,
