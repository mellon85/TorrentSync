...
public:
  ...
  typedef struct T {
    ...
    T(M &&m, N &&n)
        : message(std::forward<M>(m)), node(std::forward<N>(n)) {}
  ...
  typedef std::function<void(boost::optional<payload_type>,
  ...
  Callback(const callback_t &callback,
  ...
  void call(const dht::message::AnyMessage &, dht::Node &) const;
  ...
  bool verifyConstraints(const dht::message::AnyMessage &message) const
      noexcept;
